...
class ModelMetaClass(type):
    def __new__(cls, name, bases, attrs):
        print('ModelMetaClass __new__ name=%s' % name)
        if name=='Model':
            ...
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    if primaryKey:
                        ...
                        所以直接把StandardError改成BaseException就好了
        ...
        for k in mappings.keys():
            attrs.pop(k)
        #添加的反引号``(数字1左边的符号),是为了避免与sql关键字冲突的,否则sql语句会执行出错
...
class Model(dict, metaclass=ModelMetaClass):
    ...
    async def save(self):
        print('save() 0 __insert__:', self.__insert__)
        args = list(map(self.getValueOrDefault, self.__fields__))
        args.append(self.getValueOrDefault(self.__primary_key__))
        print('save() __insert__:', self.__insert__)
